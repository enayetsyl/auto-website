        ...
        exec(installCommand, (installError, installStdout, installStderr) => {
          ...
          exec(deployCommand, (deployError, deployStdout, deployStderr) => {
            ...
            if (vercelUrl) {
              ...
              return res.json({ success: true, message: "Deployment successful", url: vercelUrl }); // ✅ RETURN to prevent duplicate responses
            }
          
            console.error("Failed to extract Vercel deployment URL from output.");
            return res.status(500).json({ success: false, message: "Deployment succeeded, but failed to extract Vercel deployment URL." });
          });
        });
      });
    });
